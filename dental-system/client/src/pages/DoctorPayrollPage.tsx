...
import { type Doctor, type Shift, type Appointment, type Service } from '../contexts/DataContext';
...
export function DoctorPayrollPage() {
    ...
    const isLoading = doctorsIsLoading || shiftsIsLoading || appointmentsIsLoading || servicesIsLoading;

    const payrollData = useMemo(() => {
        let doctorsToProcess = doctors.filter(d => d.status === 'active');

        // If the user is a doctor, only process their own data
        if (currentUser?.role === 'Doctor') {
            // In a real app, the API would return only the relevant doctor's data.
            // Here, we filter on the client side based on the referenceId from the auth context.
            doctorsToProcess = doctorsToProcess.filter(d => d.id === currentUser.referenceId);
        }

        let data = doctorsToProcess
            .map(doc => calculateDoctorSalary(doc, targetMonth, targetYear, shifts, appointments, services))
            
        // Search is only for Admins/non-doctors
    ...
    }, [doctors, shifts, appointments, services, targetMonth, targetYear, searchTerm, currentUser])

    // Only Admin and Doctor can see this page as per navigation.ts.
    // We can add a check for other roles like Reception if they ever get here.
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-2xl font-bold text-slate-900">
                                {currentUser?.role === 'Doctor' && payrollData.length > 0 ? formatVND(payrollData[0].totalSalary) : formatVND(totalPayout)}
                            </p>
                        </div>
                    </div>
                </div>
